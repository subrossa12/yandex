...
(function (root) {
  ...
  var actions = {
    ...
    doPrestige: function () {
      ...
      /*
       * Раньше здесь показывалась полноэкранная реклама: продажа сети
       * выглядела логической паузой. Но игра идёт в реальном времени —
       * точки продолжают работать и начислять выручку, — а по п. 4.4
       * в таких играх реклама по игровому действию недопустима вообще,
       * независимо от того, насколько пауза кажется уместной. Показ
       * убран; остаются только ролики, которые игрок запускает сам.
       */
      askReviewOnce();
    },
  ...
  /*
   * Открытие новой точки — событие радостное и ровно поэтому неудачное
   * место для рекламы: игрок только что потратил накопленное и хочет
   * увидеть результат. Полноэкранный показ отсюда убран по п. 4.4 —
   * это был показ по игровому действию в игре реального времени.
   */
