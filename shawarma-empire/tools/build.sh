...
# 4. Ни одного обращения на внешний домен. Исключений нет: раньше их
#    было одно, под адрес счётчика Метрики, и из-за него архив с игрой
#    начинали отмечать антивирусы — динамическая подстановка скрипта с
#    внешнего адреса это их типовая сигнатура. Счётчик теперь
#    подключается сниппетом в index.html вручную, а билд обязан быть
#    чистым.
EXTERNAL=$(grep -rEo "https?://[a-zA-Z0-9.-]+" "$GAME" 2>/dev/null || true)
if [ -z "$EXTERNAL" ]; then
  say_ok "внешних адресов нет ни одного: ни CDN, ни шрифтов, ни счётчиков"
else
  say_fail "внешние ссылки: $(echo "$EXTERNAL" | tr '\n' ' ')"
fi

# 4a. Ни одной динамической подстановки скрипта: именно на неё
#     реагируют антивирусы, и держать её в билде нельзя даже рабочую.
INJECT=$(grep -rn "createElement('script')\|createElement(\"script\")" "$GAME" 2>/dev/null || true)
if [ -z "$INJECT" ]; then
  say_ok "динамической подстановки скриптов нет"
else
  say_fail "подстановка скрипта в: $(echo "$INJECT" | cut -d: -f1 | tr '\n' ' ')"
fi
...
# 7. Никакой полноэкранной рекламы. Игра идёт в реальном времени, и по
#    п. 4.4 показ по игровому действию в ней запрещён как класс. Один
#    отказ модерации по этому пункту уже был, поэтому проверка строгая:
#    ищем сам вызов SDK, а не наши обёртки, — вернуть его можно только
#    осознанно убрав эту проверку.
#    Строки со словом в комментарии не считаем: ищем вызов со скобкой.
FSADV=$(grep -rn "showFullscreenAdv[[:space:]]*(" "$GAME" 2>/dev/null || true)
if [ -z "$FSADV" ]; then
  say_ok "полноэкранной рекламы нет: остались только ролики по кнопке игрока"
else
  say_fail "полноэкранная реклама в: $(echo "$FSADV" | cut -d: -f1 | tr '\n' ' ')"
fi
